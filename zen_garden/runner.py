...
from .modules import BaseModule, Module, ScalingModule
from .optimization_setup import OptimizationSetup
from .plugin_system.loader import register_plugins
from .utils import ScenarioUtils, StringUtils, setup_logger

logger = logging.getLogger(__name__)


MODULES: list[type[Module]] = [BaseModule, ScalingModule]


def setup_event_controller():
    controller = EventController()

    for module_class in MODULES:
        controller.register_module(module_class)
...
def adjust_config_paths(
    config: Config, dataset, folder_output: str, config_path: str
) -> None:
...
def run(
    config="./config.json", dataset=None, job_index=None, folder_output=None
) -> OptimizationSetup | None:
    """Run ZEN-garden.
    ...
    """
    setup_logger()
    controller = setup_event_controller()

    config_path = config
    config = import_config(config)
    register_plugins(config.plugins)
    adjust_config_paths(config, dataset, str(folder_output), config_path)

    # Replace input data checks with a module
    init_event = InitEvent(config=config)
    controller.dispatch_event(init_event)
    input_data_checks = init_event.data_checks

    scenarios, model_name = prepare_scenarios(config, job_index)
    optimizer = None
    for scenario, scenario_dict in scenarios:
        # FORMULATE THE OPTIMIZATION PROBLEM
        optimizer = OptimizationSetup(config, scenario_dict, input_data_checks)
        model = ZenModel()
        steps_horizon = optimizer.get_optimization_horizon()

        for step in steps_horizon:
            StringUtils.print_optimization_progress(
                scenario, steps_horizon, step, config.system
            )

            optimizer.overwrite_time_indices(step)

            # Let the modules handle the model construction
            construct_model_event = ConstructModelEvent(
                config=config,
                zen_model=model,
                optimizer=optimizer,
                step=step,
            )
            controller.dispatch_event(construct_model_event)

            # SOLVE THE OPTIMIZATION PROBLEM
            optimizer.solve(model)

            post_optimization_event = PostOptimizationEvent(
                zen_model=model,
                optimizer=optimizer,
                step=step,
                scenario=scenario,
            )
            controller.dispatch_event(post_optimization_event)
            if post_optimization_event.should_break:
                break

            post_process_event = PostProcessEvent(
                config=config,
                zen_model=model,
                optimizer=optimizer,
                step=step,
            ...
            controller.dispatch_event(post_process_event)
